...
class Gui:
    ...
    def db_generation(self):
        ...
        def check_directory():
            ...
            text.delete(0.0, END)
            text.tag_configure('highlightline', background='grey70', borderwidth=2, relief='raised')
            ...
            text.insert(END, '\t\tСЧИТАННЫЕ ДИРЕКТОРИИ', 'red')
            text.insert(END, '\n\n')
            dir_counter = 0
            for folder in direct.keys():
                for elements in range(0, len(direct[folder])):
                    dir_counter += 1
                    text.insert(END, '\t\t\tДИРЕКТОРИЯ ' + str(dir_counter), ("green"))
                    text.insert(END, '\n')
        ...
        text = Text(frame_for_text_db_generation, width=60, height=30)
        text.pack(side=LEFT)
